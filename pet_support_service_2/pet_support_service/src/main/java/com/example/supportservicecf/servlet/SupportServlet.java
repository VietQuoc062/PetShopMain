...
public class SupportServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        try {
            ...
            SupportRequest savedRequest = supportRequestDAO.save(request);
            if (savedRequest != null && savedRequest.getId() != null) {
                resp.getWriter().write(gson.toJson(savedRequest));
            } else {
                resp.setStatus(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
                resp.getWriter().write("{\"error\":\"Không thể lưu yêu cầu hỗ trợ\"}");
            }
        } catch (Exception e) {
            e.printStackTrace();
            resp.setStatus(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
            resp.getWriter().write("{\"error\":\"Lỗi server: " + e.getMessage() + "\"}");
        }
    }
