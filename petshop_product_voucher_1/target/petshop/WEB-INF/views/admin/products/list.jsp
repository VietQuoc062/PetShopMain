<%-- admin/products/list.jsp --%> <%@ page language="java"
contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%> <%@ taglib
uri="http://java.sun.com/jsp/jstl/core" prefix="c" %> <%@ taglib
tagdir="/WEB-INF/tags" prefix="t" %>
    ...
    <c:if test="${not empty param.message}">
      <c:choose>
        <c:when test="${param.message eq 'add_success'}"
          ><div class="alert-success">Thêm sản phẩm thành công!</div></c:when
        >
        ...
        <c:when test="${param.message eq 'delete_success'}"
          ><div class="alert-success">Xóa sản phẩm thành công!</div></c:when
        >
      </c:choose>
    </c:if>
      ...
      <tbody>
        <c:forEach var="product" items="${requestScope.listProduct}">
          <tr>
            ...
            <td>
              <c:choose>
                <c:when test="${product.productCategory != null}">
            ...
            <td>
              <c:choose>
                <c:when test="${sessionScope.role == 'Owner'}">
                  <a
                    href="${pageContext.request.contextPath}/admin/products/edit?id=<c:out value='${product.productID}'/>"
                    class="btn btn-warning"
                    >Sửa</a
                  >
                  <a
                    href="${pageContext.request.contextPath}/admin/products/delete?id=<c:out value='${product.productID}'/>"
